declare module 'node-snap7' {
  ...
  export interface S7Client {
    ...
    Connect(callback: (err: Snap7Error | null, result: void) => void): void;
    ConnectSync(): void;

    ConnectTo(ip: string, rack: number, slot: number): Promise<void>;
    ConnectTo(ip: string, rack: number, slot: number, callback: (err: Snap7Error | null, result: void) => void): void;
    ...
    WriteArea(area: Area, dbNumber: number, start: number, amount: number, wordLen: WordLen, buffer: Buffer, callback: (err: Snap7Error | null, result: void) => void): void;
    ...
    ReadMultiVars(items: S7MultiVarRead[], callback: (err: Snap7Error | null, data: S7MultiVarReadResult[]) => void): void;
    ...
    WriteMultiVars(items: S7MultiVarWrite[], callback: (err: Snap7Error | null, result: S7MultiVarWriteResult[]) => void): void;
    ...
    DBRead(dbNumber: number, start: number, size: number, callback: (err: Snap7Error | null, data: Buffer) => void): void;
    ...
    Upload(blockType: BlockType, blockNumber: number, size: number, callback: (err: Snap7Error | null, data: Buffer) => void): void;
    ...
    FullUpload(blockType: BlockType, blockNumber: number, size: number, callback: (err: Snap7Error | null, data: Buffer) => void): void;
    ...
    Download(blockNumber: number, buffer: Buffer, callback: (err: Snap7Error | null, result: void) => void): void;
    DownloadSync(blockNumber: number, buffer: Buffer): void;

    Delete(blockType: BlockType, blockNum: number): Promise<void>;
    Delete(blockType: BlockType, blockNum: number, callback: (err: Snap7Error | null, result: void) => void): void;
    DeleteSync(blockType: BlockType, blockNum: number): void;

    DBFill(dbNumber: number, fillChar: number | string): Promise<void>;
    DBFill(dbNumber: number, fillChar: number | string, callback: (err: Snap7Error | null, result: void) => void): void;
    ...
    PlcHotStart(callback: (err: Snap7Error | null, result: void) => void): void;
    PlcHotStartSync(): void;

    PlcColdStart(): Promise<void>;
    PlcColdStart(callback: (err: Snap7Error | null, result: void) => void): void;
    PlcColdStartSync(): void;

    PlcStop(): Promise<void>;
    PlcStop(callback: (err: Snap7Error | null, result: void) => void): void;
    PlcStopSync(): void;

    // Control Functions
    CopyRamToRom(timeout: number): Promise<void>;
    CopyRamToRom(timeout: number, callback: (err: Snap7Error | null, result: void) => void): void;
    CopyRamToRomSync(timeout: number): void;

    Compress(timeout: number): Promise<void>;
    Compress(timeout: number, callback: (err: Snap7Error | null, result: void) => void): void;
    CompressSync(timeout: number): void;

    PlcStatus(): Promise<PlcStatus>;
    PlcStatus(callback: (err: Snap7Error | null, status: PlcStatus) => void): void;
    ...
    SetSessionPassword(password: string, callback: (err: Snap7Error | null, result: void) => void): void;
    SetSessionPasswordSync(password: string): void;

    ClearSessionPassword(): Promise<void>;
    ClearSessionPassword(callback: (err: Snap7Error | null, result: void) => void): void;
    ClearSessionPasswordSync(): void;

    // DateTime Functions
    GetPlcDateTime(): Promise<Date>;
    GetPlcDateTime(callback: (err: Snap7Error | null, date: Date) => void): void;
    GetPlcDateTimeSync(): Date;

    SetPlcDateTime(date: Date | DateTimeObject): Promise<void>;
    SetPlcDateTime(date: Date | DateTimeObject, callback: (err: Snap7Error | null, result: void) => void): void;
    SetPlcDateTimeSync(date: Date | DateTimeObject): void;

    SetPlcSystemDateTime(): Promise<void>;
    SetPlcSystemDateTime(callback: (err: Snap7Error | null, result: void) => void): void;
  ...
  export interface S7Server extends EventEmitter {
    ...
    on(event: 'error', listener: (err: Snap7Error) => void): this;

    // Error Constants
    errSrvCannotStart: number;
    errSrvDBNullPointer: number;
    errSrvAreaAlreadyExists: number;
    errSrvUnknownArea: number;
    errSrvInvalidParams: number;
    errSrvTooManyDB: number;
    errSrvInvalidParamNumber: number;
    errSrvCannotChangeParam: number;
    ...
    evcListenerCannotStart: number;
    evcClientAdded: number;
    evcClientRejected: number;
    evcClientNoRoom: number;
    evcClientException: number;
    evcClientDisconnected: number;
    evcClientTerminated: number;
    evcClientsDropped: number;
    evcPDUincoming: number;
    evcDataRead: number;
    evcDataWrite: number;
    evcNegotiatePDU: number;
    evcReadSZL: number;
    evcClock: number;
    evcUpload: number;
    evcDownload: number;
    ...
    // Event Subcodes
    evsUnknown: number;
    evsStartUpload: number;
    evsStartDownload: number;
    evsGetBlockList: number;
    evsStartListBoT: number;
    evsListBoT: number;
    evsGetBlockInfo: number;
    evsGetClock: number;
    evsSetClock: number;
    evsSetPassword: number;
    evsClrPassword: number;

    // Event params: function groups
    grProgrammer: number;
    grCyclicData: number;
    grBlocksInfo: number;
    grSZL: number;
    ...
    CodeControlUnknown: number;
    CodeControlColdStart: number;
    CodeControlWarmStart: number;
    CodeControlStop: number;
    CodeControlCompress: number;
    CodeControlCpyRamRom: number;
    CodeControlInsDel: number;

    // Event results
    evrNoError: number;
    evrFragmentRejected: number;
    evrMalformedPDU: number;
    evrSparseBytes: number;
    evrCannotHandlePDU: number;
    evrNotImplemented: number;
    evrErrException: number;
    evrErrAreaNotFound: number;
    evrErrOutOfRange: number;
    evrErrOverPDU: number;
    evrErrTransportSize: number;
    evrInvalidGroupUData: number;
    evrInvalidSZL: number;
    evrDataSizeMismatch: number;
    evrCannotUpload: number;
    evrCannotDownload: number;
    evrUploadInvalidID: number;
    evrResNotFound: number;
